    ...
    function get_content() {
		...
		<div class="container">
            ...
            for ($i = 1; $i <= $data->slidesnumber; $i++) {
                ...
                <div class="item">
                  <div class="home-text">';
                  $text .='<h2><span data-ccn="slide_title'.$i.'">'.format_text($data->$slide_title, FORMAT_HTML, array('filter' => true)).'</span>';
                  if(!empty($data->$slide_line_break) && $data->$slide_line_break == '1'){
                    $text .='<br>';
                  }
                  $text .='<span data-ccn="slide_title_2'.$i.'" class="text-thm">'. format_text($data->$slide_title_2, FORMAT_HTML, array('filter' => true)).'</span>';
                  $text .='</h2>';
                  ...
                  if(!empty($data->$slide_btn_text) && !empty($data->$slide_btn_url)){
                    $text .='<a data-ccn="slide_btn_text'.$i.'" target="'.$slide_btn_target.'" class="btn home_btn" href="'.format_text($data->$slide_btn_url, FORMAT_HTML, array('filter' => true)).'">'.format_text($data->$slide_btn_text, FORMAT_HTML, array('filter' => true)).'</a>';
                  }
                  $text .='
                  </div>
                </div>';
